...
1. Install it, or 
2. Uncomment the R part in the `single-es.yml` file.

For the docker way, add a path to a data folder if you need persistent data (in `volumes` section).

Use the same command to launch the service and use the R command-line interface inside the container.
...
➜ docker ps
CONTAINER ID   IMAGE                                                  COMMAND                  CREATED          STATUS          PORTS                              NAMES
0f1afd07f58a   rocker/r-ver:4.0.3                                     "/bin/bash"              4 minutes ago    Up 4 minutes                                       r4
40814036d980   docker.elastic.co/elasticsearch/elasticsearch:7.10.2   "/tini -- /usr/local…"   43 minutes ago   Up 12 minutes   0.0.0.0:9200->9200/tcp, 9300/tcp   elasticsearch

# connect to the R container with its ID (previous command)
➜ docker exec -it 0f1afd07f58a bash

# inside the R container, install cURL
root@0f1afd07f58a:/$ apt update && apt install -y curl
...
...
### Save the R docker state

Use the `docker commit` command to save the current container state in a new image (e.g all the installed packages and the last commands run).
...
# Outside of any container, create a new R image (i.e. "myrepo/my_r_image") 
# with version 0.1.0 from the current running R container (i.e. "0f1afd07f58a")
➜ docker commit 0f1afd07f58a myrepo/my_r_image:0.1.0
```

With this new image, you can change the `image: rocker/r-ver:4.0.3` line in the `single-es.yml` file to start with the saved state the next time you start with docker-compose. 

See the [Docker documentation](https://docs.docker.com/engine/reference/commandline/commit/) for more information.
...
Stopping r4                         ... done
Stopping 40814036d980_elasticsearch ... done
Removing r4                         ... done
Removing 40814036d980_elasticsearch ... done
