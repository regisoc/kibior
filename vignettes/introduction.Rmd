...
<center>
![](./img/kibior_principle.svg){ width=100% }
</center>
...
message("Getting vignette variables...")
...
message("Variable added: es_endpoint=", es_endpoint)
# port
es_port <- tryCatch(
    {
        env_es_port %>%
...
message("Variable added: es_port=", es_port)
# args
args <- list(
    host = es_endpoint, 
    port = es_port,  
...
message("Variable added: es_username=", es_username)
message("Variable added: es_password=", es_password)
#
...
delete_if_exists("test_index_single")
dplyr::storms %>% 
    dplyr::select(name) %>% 
    unique() %>% 
    as.list() %>% 
...
Here, we will see the main methods (`push()`, `pull()`, `list()`, `fields()`, `has()`, `export()`, `import()`, `move()`, `copy()`) and public attributes (verbosity) of `Kibior` class. Kibior uses [`elastic`](https://docs.ropensci.org/elastic/) [@ref_elastic] to perform base functions.
...
- `$quiet_progress` toggles the printing of progress bars: the printing is on by default. This can be useful for scripts.
- `$quiet_results` toggles the verbosity output of called methods. It is `FALSE` by default which means methods will print out result. You may want to deactivate it when you do not need interactive feedback.
...
> 2. `$push()` expects well-formatted data, mainly in a data.frame or derivative structure such as tibble.
...
## `$fields()`: List all fields of an Elasticsearch index
...
Note: `Kibior` is using `rio` package [@ref_rio] that can export much more formats. 
...
The `$import()` method can natively manage alignement and feature formats (e.g. fasta, bam, gtf, gff, bed, etc.) since it also wraps Bioconductor library methods such as [`rtracklayer::import()`](https://doi.org/doi:10.18129/B9.bioc.rtracklayer) [@ref_rtracklayer], [`Biostrings::read*StringSet()`](https://doi.org/doi:10.18129/B9.bioc.Biostrings) [@ref_biostrings] and [`Rsamtools::scanBam()`](https://doi.org/doi:10.18129/B9.bioc.Rsamtools) [@ref_rsamtools] as optional import packages.

Dedicated methods are implemented inside Kibior (e.g. `$import_features()` and `$import_alignments()`), and the generic `$import()` method tries to open the right format according to file extension. See methods `import_*()` manual pages.
...
m <- kc$move(from_index = "storms_file", to_index = "storms_file_moved")
kc$list()
```
...
```{r copy}
#> copy index
m <- kc$copy(from_index = "storms_file_moved", to_index = "storms_file")
kc$list()
```
...
s <- dplyr::starwars %>% kc$push("starwars")
s <- dplyr::storms %>% kc$push("storms", mode = "recreate")
...
s <- ggplot2::diamonds %>% kc$push("diamonds")
...
#> consider this dataset
s <- dplyr::storms %>% kc$push("storms", mode = "recreate")
s <- dplyr::starwars %>% kc$push("starwars", mode = "recreate")
```
...
- `dplyr::select()` with `field` parameter,
...
Kibior supports query parameter when joining to accelerate data retrival time but *cannot join on listed fields*.
...
As yo ucan see, Kibior uses suffixes "left" and "right" on data column.
...
- `$move()` and `$copy()` for remote instances are very sensitive to authentication and security configurations. Some tasks will not be possible due to each organism security measures.
...
|-----------------|---------------------|-----------------------|
| `Elasticsearch` | `6.8.6`, `7.5.1`    |                       |
| `R`             | `3.6.1`             |                       |
| `RStudio`       | Version 1.2.5001    | Build 93, 7b3fe265    |
