...
kc$stats(c("sw", "s*"), c("height", "mass"))

#> work also with query and sigma for standard deviation
kc$stats("sw", c("height", "mass"), sigma = 2.5, query = "homeworld:naboo")
```
...
## `$min()`: get the minimum of some columns
...
## `$max()`: get the maximum of some columns
...
## `$avg()`: get the average of some columns
...
## `$sum()`: get the sum of some columns
