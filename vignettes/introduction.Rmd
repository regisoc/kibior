...
delete_if_exists <- function(index_name){
    hi <- kc$has(index_name)
    # message("index searched: ", index_name, ", found: ", hi)
    hi[hi == TRUE] %>%
        names() %>% 
        kc$delete()
}
...
Here, we will see the main methods (`push()`, `pull()`, `list()`, `columns()`, `keys()`, `has()`, `export()`, `import()`, `move()`, `copy()`) and public attributes (verbosity) of `Kibior` class. Kibior uses [`elastic`](https://docs.ropensci.org/elastic/) [@ref_elastic] to perform base functions.
...
The `$import()` method can natively manage sequence, alignement and feature formats (e.g. fasta, bam, gtf, gff, bed, etc.) since it also wraps Bioconductor library methods such as [`rtracklayer::import()`](https://doi.org/doi:10.18129/B9.bioc.rtracklayer) [@ref_rtracklayer], [`Biostrings::read*StringSet()`](https://doi.org/doi:10.18129/B9.bioc.Biostrings) [@ref_biostrings] and [`Rsamtools::scanBam()`](https://doi.org/doi:10.18129/B9.bioc.Rsamtools) [@ref_rsamtools] as optional import packages.
...
By adding one line to the `elasticsearch.yml` configuration file, allowing a server whitelist, Elasticsearch servers can talk to ech others.
By this, they can transfer data across them in a much faster way and secure way.
...
- The `columns` parameter of the `$search()` and `$pull()` methods does not handle metadata columns.
